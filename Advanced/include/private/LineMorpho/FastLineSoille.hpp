...
#define PI 3.14159265358979323846
  ...
  private:
    ...
    ulong ComputeLineNegDiag(long x, long y, ulong width, ulong height,
                             ulong *p)
    {
      ulong idx;
      long  x0;
    ...
    ulong ComputeBresenhamLineNX(long x, long y, long dx, long dy, ulong phase,
                                 ulong width, ulong height, ulong *p)
    ...
    ulong ComputeBresenhamLineNY(long x, long y, long dx, long dy, ulong phase,
                                 ulong width, ulong height, ulong *p)
    /* Computes pixel coords along line with slope m<-1 */
    /* Returns # of pixel coords (num) written to array p (num <= height) */
    {
    ...
    void DilateLine(T *f, SMIL_UNUSED ulong width, SMIL_UNUSED ulong height,
                    ulong k, ulong nx, ulong *p, T *g, T *h, T *h2, T *r)
    /* k is length of SE in number of pixels */
  ...
  public:
                     ...
                     Image<T> &imOut)
    {
      ...
      ImageGrayDilateLine(&MyImgIn, r * 2 + 1, dx, dy, 1, p, g, h, h2,
                          &MyImgOut);
