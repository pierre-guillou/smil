...
namespace smil {
  ...
  private:
    ...
    inline void BuildOpeningFromPoint(
      T F, int Ind, Node *MyStack, int *stackSize, T *bufferOut) {
      LineIdx[wp] = Ind;
    ...
    int ComputeLinePosDiag(T *bufferIn,
                           int W,
                           int H,
                           int x,
                           int y,
      ...
      for(; (x < W) && (y < H); x++) {
        BuildOpeningFromPoint(
          bufferIn[idx], idx, MyStack, stackSize, bufferOut);
        idx += W + 1;
        y++;
      }
      return (x - x0);
    ...
    int ComputeLineNegDiag(T *bufferIn,
                           int W,
                           int H,
                           int x,
                           int y,
    ...
    int ComputeBresenhamLinePX(T *bufferIn,
                               int W,
                               int H,
                               int x,
                               int y,
    ...
    int ComputeBresenhamLineNX(T *bufferIn,
                               int W,
                               int H,
                               int x,
                               int y,
    ...
    int ComputeBresenhamLinePY(T *bufferIn,
                               int W,
                               int H,
                               int x,
                               int y,
    ...
    int ComputeBresenhamLineNY(T *bufferIn,
                               int W,
                               int H,
                               int x,
                               int y,
    ...
    void LineOpeningHorz(T *bufferIn, int W, int H, int radius, T *bufferOut) {
      ...
      for(j = 0; j < H; j++) // Pour toutes les lignes
      {
        ...
        for(i = 1; i < W; i++) // Pour toutes les colonnes
          BuildOpeningFromPoint(
            bufferIn[i + j * W], i + j * W, MyStack, &stackPos, bufferOut);
    ...
    void LineOpeningDiag(
      T *bufferIn, int W, int H, int dx, int dy, int radius, T *bufferOut) {
      size = radius * 2 + 1;
      int y, x, nx;
      ...
      if(abs(dx) == abs(dy)) {
        if(dx == -dy) {
          y = 0;
          do {
            nx = ComputeLineNegDiag(
              bufferIn, W, H, 0, y++, MyStack, &stackPos, bufferOut);
            EndProcess(MyStack, &stackPos, bufferOut);
          } while(nx > 0);
        ...
        if(((dx > 0) && (dy > 0)) || ((dx < 0) && (dy < 0))) {
          ...
          y = H - 1;
          do {
            nx = ComputeBresenhamLinePX(
              bufferIn, W, H, 0, y--, dx, dy, MyStack, &stackPos, bufferOut);
            EndProcess(MyStack, &stackPos, bufferOut);
          } while(nx > 0);
        } else {
          dx = abs(dx);
        ...
        if(((dx > 0) && (dy > 0)) || ((dx < 0) && (dy < 0))) {
          ...
          x = W - 1;
          do {
            nx = ComputeBresenhamLinePY(
              bufferIn, W, H, x--, 0, dx, dy, MyStack, &stackPos, bufferOut);
            EndProcess(MyStack, &stackPos, bufferOut);
          } while(nx > 0);
        } else {
          dx = abs(dx);
  ...
  public:
    RES_T ImFastLineOpen(const Image<T> &imIn,
                         const int angle,
                         const int radius,
                         Image<T> &imOut) {
      // Check inputs
