...
#define localesT vector<vector<UINT>>
  ...
  double radius_dist(IntPoint p, IntPoint q) {
    ...
    double pmag = std::sqrt(double(p.x * p.x + p.y * p.y + p.z * p.z));
    double px = p.x / pmag;
    double py = p.y / pmag;
    double pz = p.z / pmag;
    double qmag = std::sqrt(double(q.x * q.x + q.y * q.y + q.z * q.z));
    double qx = q.x / qmag;
    double qy = q.y / qmag;
    double qz = q.z / qmag;
  ...
  void generateReachables(localesT &reachables,
                          const localesT &neighbors,
                          const StrElt &s) {
    StrElt se = s.noCenter();
