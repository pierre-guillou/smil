...
namespace smil {
  template <class T>
  RES_T
    kuwaharaFilter(const Image<T> &imIn, const int radius, Image<T> &imOut) {
    ...
    for(int y1 = -offset; y1 < H; y1++) {
      for(int x1 = -offset; x1 < W; x1++) {
        ...
        mean[x1 + offset + (y1 + offset) * width2] = (double)(sum / (double)n);
  ...
  RES_T kuwaharaFilterRGB(const Image<T1> &imIn,
                          const int radius,
                          Image<T2> &imOut) {
    return RES_OK;
  }
