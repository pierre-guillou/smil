...
#define WINDOW_SIZE 7
  ...
  void debed(T *imOut,
    ...
    for(i = size; i < NewW - size; i++)
      for(j = size; j < NewH - size; j++)
        imOut[(i - size) + (j - size) * W] = (T)Img[i + j * NewW];
  }
  ...
  //**************************************************************************
  void ApplySmoothing_Horizontal(double *x,
                                 double *y,
                                 double *Causal,
                                 double *AntiCausal,
                                 int W,
    ...
    for(j = 0; j < H; j++) {
      ...
      for(i = W - 3; i >= 0; i--)
        AntiCausal[i + j * W]
          = (double)(g * a1 * c1 * x[i + 1 + j * W] - g * a2 * x[i + 2 + j * W]
  ...
  //**************************************************************************
  void ApplySmoothing_Vertical(double *x,
                               double *y,
                               double *Causal,
                               double *AntiCausal,
                               int W,
  ...
  //**************************************************************************
  bool IsCandidateEdge(
    bool *Buff, double *Orig, int W, SMIL_UNUSED int H, int i, int j) {
    ...
    if(Buff[i + j * W] == 1 && Buff[i + 1 + j * W] == 0)
      return (
        Orig[i + 1 + j * W] - Orig[i - 1 + j * W] > 0 ? 1 : 0); // positive z-c
      ...
      return (
        Orig[i + 1 + j * W] - Orig[i - 1 + j * W] < 0 ? 1 : 0); // negative z-c
  ...
  //**************************************************************************
  double ComputeAdaptativeGradient(
    bool *Bli, double *Orig, int W, SMIL_UNUSED int H, int k, int l) {
    int i, j;
    double SumOn, SumOff;
    double AvgOn, AvgOff;
    int NbOn, NbOff;
  ...
  //**************************************************************************
  void LocateZeroCrossings(
    double *Orig, double *BufFiltrer, bool *ImgBli, int W, int H) {
    int i, j;
